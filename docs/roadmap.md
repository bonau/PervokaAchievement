...
`deliver_later` 已經負責郵件。Toast 在 v1.4 之前維持下次請求才讀 `notified_at`，與背景給獎相容。同一頁即時跳出（WebSocket 或同等伺服器推送）排在 v1.4，不阻擋條件引擎這條主線。
...
| 即時提示 | **v1.4**。v1.1–v1.3 繼續用下次頁面載入的 toast。架構主線先做完，再補同一頁即時跳出。 |

安全邊界是 JSON 綱要與允許清單。v1.1 不開放管理介面編輯條件，避免在引擎尚未存在時就把規則寫進資料庫。

| 版本 | 內容 |
|------|------|
| v1.1 | 領域事件與 ActiveJob。內建成就改為訂閱。管理介面不變。Toast 仍是下次請求讀 `notified_at`。 |
| v1.2 | 規則儲存、結構化規則引擎、管理介面。管理者可新增成就與條件。 |
| v1.3 | 內建成就依序改成種子規則。文件把子類別寫法標成相容路徑。 |
| v1.4 | 同一頁即時解鎖提示。背景給獎完成後，已開啟的頁面立刻跳出 toast。 |

### v1.4 即時提示

v1.0 的 toast 是 view hook 在**下一次頁面載入**讀未通知的成就。給獎改成 ActiveJob 之後，這條路徑仍然正確，只是使用者若留在同一頁、又還沒再導向，就看不到剛解鎖的提示。

v1.4 補上伺服器主動推送，讓已開啟的頁面在工作完成後立刻顯示既有 toast。範圍：

- 做：同一頁即時 toast（WebSocket 或同等的伺服器推送，實作時再選傳輸方式）。
- 不做：瀏覽器／作業系統的 Push API 系統通知、推播權限提示、離線通知。
- 不做：改郵件通路。`deliver_later` 維持現狀。
- 相容：沒有即時通道時，下次頁面載入的 toast 仍要能用。
